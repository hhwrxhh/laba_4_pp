...
class CartSchema(ma.Schema):
    ...
    fk_user_id = fields.Integer(load_only=True)
    fk_order_id = fields.Integer(load_only=True)
    # cart_dosed = fields.Nested()
    dosed = fields.Nested(DosedSchemaGet, dump_only=True)


class CartSchemaAdd(ma.Schema):
    fk_dosed_id = fields.Integer(load_only=True, required=True)
class CartSchemaDosedGet(ma.Schema):
    dosed = fields.Nested(DosedSchemaGet, dump_only=True)
...
class CartSchemaGet(ma.Schema):
    ...
    dosed = fields.Nested(CartSchemaDosedGet, dump_only=True)
